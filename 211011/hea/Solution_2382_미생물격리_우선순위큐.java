...
public class Solution_2382_미생물격리_우선순위큐 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int test_case = 1; test_case <= T; test_case++) {
			...
			// 격리 시간 M만큼
			for (int m = 0; m < M; m++) {
				PriorityQueue<Microbe> microbes = new PriorityQueue<>();						 // 살아있는 군집들 관리
				for (int i = 0; i < list.size(); i++) {						     // 군집 이동
					Microbe micro = list.get(i);
					int d = micro.d;
					int num = micro.num;
				...
				for (int i = 0; i < N; i++) {	
					for (int j = 0; j < N; j++) {
						if (map[i][j] != null) {							 // 맵 위에 군집이 있으면
							list.add(map[i][j]);							 // 군집 관리 리스트에 추가
						}
					}
				}
			}
